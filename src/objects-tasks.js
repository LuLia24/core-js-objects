...
function compareObjects(obj1, obj2) {
  const keys1 = Object.keys(obj1); // [1-2]
  const values1 = Object.values(obj1);
  const keys2 = Object.keys(obj2); // [1-3]
...
function makeWord(lettersObject) {
  // Преобразуем объект в массив пар [позиция, буква]
  const lettersWithPositions = Object.entries(lettersObject).flatMap(
    ([letter, positions]) => positions.map((pos) => [pos, letter])
  );
  const sortedLetters = lettersWithPositions.sort((a, b) => a[0] - b[0]);
